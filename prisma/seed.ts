...
async function main() {
  ...
  // Order 1: REQUESTED status (pending approval)
  await prisma.order.create({
    data: {
      clientId: client1.id,
      siteId: site1.id,
      deliveryAddress: '123 KN 5 Ave, Kigali',
      ...
      items: {
        create: [
          {
            productId: products[0].id, // Fresh Milk
            quantityKg: 20,
            unitPrice: 1000,
            subtotal: 20000,
          },
          {
            productId: products[1].id, // Cheese
            quantityKg: 2,
            unitPrice: 5000,
            subtotal: 10000,
          },
  ...
  await prisma.order.create({
    data: {
      clientId: client1.id,
      siteId: site1.id,
      deliveryAddress: '456 KG 10 St, Kigali',
  ...
  // Order 3: REJECTED status
  await prisma.order.create({
    data: {
      clientId: client2.id,
      siteId: site2.id,
      deliveryAddress: '789 Musanze Road',
      notes: 'Large order for restaurant',
      totalAmount: 100000,
      status: 'REJECTED',
      rejectedAt: new Date(),
      ...
      items: {
        create: [
          ...
          {
            productId: products[3]?.id || products[1].id,
            quantityKg: 10,
            unitPrice: 4500,
            subtotal: 45000,
          },
  ...
  // Order 4: Another REQUESTED order
  await prisma.order.create({
    data: {
      clientId: client2.id,
      siteId: site2.id,
      deliveryAddress: '321 Rubavu Street',
