...
export class SitesController {
  ...
  @ApiResponse({ status: 201, description: 'The site has been successfully created.', type: SiteEntity })
  @ApiResponse({ status: 400, description: 'Bad request.' })
  async create(@Body() createSiteDto: CreateSiteDto): Promise<{ status: string; message: string; data: SiteEntity }> {
    try {
...
@ApiResponse({ status: 200, description: 'Successfully retrieved all sites.', type: [SiteEntity] })
...
async findOne(@Param('id') id: string): Promise<{ status: string; message: string; data?: SiteEntity }> {
...
@ApiResponse({ status: 200, description: 'The site has been successfully updated.', type: SiteEntity })
@ApiResponse({ status: 404, description: 'Site not found.' })
async update(
  @Param('id') id: string,
