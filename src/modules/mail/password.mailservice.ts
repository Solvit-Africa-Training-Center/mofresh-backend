...
export class PasswordEmailService {
  ...
  async sendClientCredentials(email: string, password: string, firstName: string): Promise<void> {
    if (!this.emailEnabled) {
      this.logger.log(`📧 [SKIPPED] Would send credentials to: ${email} (password: ${password})`);
      return;
    }
    ...
    try {
      ...
      this.logger.log('✅ Client credentials email sent to: ' + email);
    } catch (error) {
      const err = error as Error;
      this.logger.error('❌ Email delivery failed: ', err.message);
    }
  }
}
